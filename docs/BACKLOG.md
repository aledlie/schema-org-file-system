...
7. ~~**Named-insurer / business-plan shadow flips + letterhead regression**~~ — **DONE (`a18be52` + `7f2e7d2`, 2026-07-17).** A fresh `~/Downloads` shadow run (41 records, 36/41 = 87.8% agreement) surfaced two *correct* unified flips now pinned as goldens: a named property/casualty insurer without bank/wire/loan indicators files `financial/insurance` (`text_content` outscores `organization_keyword` — the USAA-Casualty case; `named_casualty_insurer_routes_financial_insurance`), and a `.md` business plan files `business/planning`, content beating the extension-only filepath prior (`business_plan_markdown_routes_business_planning`). Investigating the golden suite also caught a regression from the unreviewed parallel commit `7280518`: demoting `known_text_companies` to a no-topical-scores fallback (needed so "Bill to: Integrity Studio" stops mis-filing the vendor invoice) had regressed `org_healthcare_letter` (a Leora Home Health letter lost to its medical vocabulary → `medical/prescriptions`). Fixed with a letterhead-position rule — a curated known company named in the first `_KNOWN_COMPANY_LETTERHEAD_CHARS` (60) chars is the filing subject and overrides topic; named later it stays a passing mention (Option C). Golden suite 41/41.
