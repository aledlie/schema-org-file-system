...
- **Still hand-enumerated: the three `.tech-badge` filters** in `should display technology stack in footer`, which also use substring `hasText`. Same shape, not fixed — it is a footer badge list, so drift is lower-stakes, but it is the last instance in this file.
- **Unfixed and outside the test suite: `_site/index.html` has the same bug in CSS.** The card entry animation is staggered by `.feature-card:nth-child(1..4)`; it stops at 4, so the fifth card renders with no `animation-delay`. Identical root cause — a hardcoded per-card enumeration the new card did not update — and no test would catch it. `_site/index.html` is regenerated by content runs, so the fix belongs in the generator or in a `:nth-child(n)` formula, not a hand-added rule.
...
- **The `np.asarray` guard added alongside that fix is unreachable, and the reason is import order.** `except (ImportError, NameError, AttributeError)` wraps the `np.asarray(heic_img)` call for "PIL available but numpy not installed" (`ocr_classifier.py:325`). That state cannot occur: `numpy` and `PIL` are imported in **one** `try` block (`:66-72`) with **numpy first**, so a missing numpy aborts before `Image`/`ImageOps` bind, `_load_rgb` raises `NameError` internally, returns `None`, and `_run_image_ocr` early-returns at the `heic_img is None` branch — never reaching the guarded line. Harmless, and cheap to leave. **It stops being unreachable the moment someone reorders those two imports or splits them into separate `try` blocks**, which is exactly the kind of tidy-up that looks safe — so remove the guard and the ordering coupling together, or keep both and say so. (Recorded because the guard's own comment asserts a scenario the module's import structure rules out; a future reader will otherwise trust the comment over the imports.)
