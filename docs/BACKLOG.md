...
Verified-genuine gaps from the 2026-07-13 salvage review of the orphaned `add_content_descriptions.py` / `analyze_renamed_files.py` / `image_renamer_metadata.py` chain (28-agent adversarial workflow: 3 inventory readers, per-capability refutation against `src/` + `scripts/shared/`, synthesis). The GPS/EXIF portion landed first (see the metadata-extraction item above); items 1 and 2 landed 2026-07-13.

**Status:** Done — all capabilities landed 2026-07-13; item 2's eval follow-up still open (below)
**Priority:** P3
**Source:** dead-script salvage review, 2026-07-13

1. **Classifier-derived schema.org `description`** — **Done** 2026-07-13 (`394956c` + export-path completion, same day). Landed as specified: `_run_clip_signal` stashes the winning CLIP label+score into `_last_file_state` (even below the enhancement threshold — the description states its confidence), `FileProcessor._content_description` composes `"Content: {label} ({confidence:.0%} confident)"` in `generate_schema` with filename fallback for non-CLIP paths; the "Text detected: …" OCR half deliberately not ported (duplicates `abstract`). Golden `image_with_clip_description.json` recorded. The export gap in the premise is closed too: `build_file_jsonld` now emits `description` from the persisted `schema_data` blob (File has no description column) and the core streaming exporter selects `schema_data`, so REST/bulk exports carry it — pinned by `test_core_file_export_emits_schema_data_description` and `TestFileModel` description tests. Legacy rows export their recorded filename-valued description; rows without `schema_data` omit the property. Note: the snapshot-pairing with the fidelity-losses item didn't happen — `394956c` recorded its golden independently, so resolving fidelity-losses will trip the snapshots a second time.
2. **Screenshot OCR title-snippet naming** — **Done** 2026-07-13 (commit `cc0e77c`). Landed as specified: `title_snippet_from_lines()` in `scripts/shared/filename_utils.py` (top-3 lines, 10<len<50, sanitized, 40-char cap), line-preserving `extract_screenshot_lines()` in `ocr_classifier.py`/`ocr_easyocr.py` (existing `extract_screenshot_text` flattening untouched — digit detection unaffected), wired into `rename_images.py`'s screenshot profile with fallback to CLIP-label naming when no line qualifies; EXIF-datetime fallback deliberately not ported. Unit + integration green (1117 + 110, incl. new `tests/unit/test_rename_images.py`). **Open follow-up:** the eval gate the original item required was NOT run before landing — the UI-chrome risk of the 3-line window (repo policy: eval-gated naming changes) is unverified against real screenshots. Run a before/after sample of the screenshot renamer (or the eval suite) and revert/tighten the window if chrome lines dominate.

All three scripts were deleted 2026-07-13 ahead of the ports (reference implementations remain recoverable from pre-deletion git history). `CONTENT_ABBREVIATIONS` survives in `shared/constants.py`. Also rejected by the review: OCR-keyword filename enrichment (reading-order-naive — grabs UI chrome; KIE + entity_detector already extract the valuable signals), filename-token sanitizer (no live path embeds free text in filenames), GPS location in generated filenames (content-naming was the chosen policy).
