...
- **Second, narrower blindness in the same context: `image_metadata_provider` is never wired** (`backtest_scoring.py:370-378` passes text/ocr/clip/kie only), so `ctx.ensure_image_metadata()` is always `{}` and `ClipVisionSignal`'s GPS → `GPS_TRAVEL_CATEGORY` upgrade cannot fire in replay. Unlike the analyzer this one is cheap to fix — EXIF is on disk and the files resolve.
