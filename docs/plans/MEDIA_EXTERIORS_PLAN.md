...
- **Screenshot provenance reroute (2026-07-18):** a scene-class win on a screenshot-named file refiles under `Media/Photos/Screenshots/{Interiors,Exteriors,Places}` (taxonomy keys `photos_screenshots_{interiors,exteriors,places}`) while keeping the scene `@type` — provenance-first foldering, content-first typing (`SCREENSHOT_SCENE_SUBCATEGORY` + `ContentOrganizer._reroute_screenshot_scene`, gated on `is_screenshot_named`). `Media/{Interiors,Exteriors,Place}` now hold first-party photos/renders only; renamed/cropped screenshots miss the filename gate and keep scene placement by design. `graphics_other` is deliberately excluded.
