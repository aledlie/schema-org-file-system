...
- **`scripts/d1/schema.sql` is generated — regenerate it after any model change** — `python scripts/d1/generate_schema.py` renders it from `Base.metadata`; nothing enforces that, and it was found stale across three prior model changes on 2026-07-27 (the category index swap, the `people` validation columns, `file_categories.signal_evidence`). See [`docs/BACKLOG.md`](docs/BACKLOG.md).
