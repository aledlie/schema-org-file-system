...
- **Backtest replay oracle** — `files.image_classification` (CLIP scores) is written only by `scripts/backfill_clip_scores.py`, never by production; re-run it after new content runs. The replay classifies under `original_path` (pre-move), so anything keyed on the file's on-disk identity must resolve `current_path` first: `SceneSignal` needs the harness's context-path→`current_path` map to find cached embeddings, and `make_image_metadata_provider` does the same fallback to read EXIF. Stored decisions are a biased oracle (manual corrections, pre-unified placements) — trust the non-media slice; media-row disagreements may be fidelity artifacts. Some stored labels are simply wrong — inspect actual rows (`winning_signals`, OCR text) before "fixing" a signal for a disagreement.
- **The replay reconstructs from stored columns — except EXIF, which has no stored value to reconstruct** — `files` declares `exif_datetime`, `gps_latitude` and `gps_longitude`, but **nothing writes them** (empty on all 488 rows), so a column-based provider returns `{}` corpus-wide and looks like a working fix. `build_context` therefore reads the file (`scripts/backtest_scoring.py`), returning only the two keys any signal consumes — `datetime` and `gps_coordinates`. **Do not "complete" it by calling `get_metadata_summary`:** that reverse-geocodes through Nominatim, putting a network call in `make calibrate` for fields (`location_name`, `year`, `month`, `date_str`, `text_metadata`) no signal reads. Wiring it changed 3 of 488 decisions and moved agreement −1 — **all of which was the oracle being stale, confirmed per row**; treat a small agreement drop here as a prompt to open the rows, not as a regression.
