...
| Core-query export path | `SchemaOrgExporter(use_core=True)` is the **default**: bulk-selects columns + association tables and serializes via the shared `build_*_jsonld` pure functions in `models.py`, skipping ORM hydration (~3× faster on bulk export, byte-identical, parity-locked by `tests/integration/test_core_export_parity.py`). Each `to_schema_org()` is a thin delegator to those builders — **edit the builders, not the methods**, or the ORM and Core paths diverge. Relationship-order parity relies on natural association-row order (first category → `mainEntityOfPage`); do not add `ORDER BY` to `_load_file_refs`. The production path covers relationships (categories/companies/people/locations, Category parent/subcategory) + File image/GPS fields. Exports **stream** (`_stream_array` + lazy `_iter_records`; File path column-selects + `yield_per`) so memory stays flat (~9× less than materializing) at 265k scale — don't reintroduce a full `records` list or `json.dumps(whole_doc)`. `export_entities_filtered` uses Core for File/Company/Person/Location (subset-scoped refs via `_load_file_refs(file_ids=…)`); only Category + unknown types still fall back to ORM. |
