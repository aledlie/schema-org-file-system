...
- **`FORCE_COLOR` breaks CLI-output test asserts + tool-output greps** — this machine's shell profile exports `FORCE_COLOR=3`, and Python 3.14 argparse colorizes help/usage/error text even under pytest capture, so plain-substring asserts (`"usage: organize-files" in out`) fail. `NO_COLOR` takes precedence: the `run_cli` helper in `tests/integration/test_cli.py` sets `NO_COLOR=1` via monkeypatch — route any test asserting on CLI/argparse output through it. Same env also colorizes piped `mypy`/`pytest` output, so `grep " error: "` over tool output silently matches nothing — strip ANSI or run with `env -u FORCE_COLOR` first.
...
- **`categories.name` is globally UNIQUE** — you cannot create `personal/contacts` while `person/contacts` exists (same leaf name). Taxonomy migrations must UPDATE the existing category row in place (`full_path`, `parent_id`, `canonical_id` = uuid5(cat-ns, full_path)), not create-and-repoint.
