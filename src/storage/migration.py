...
class JSONMigrator:
    ...
    def __init__(self, db_path: Union[str, Path] = DEFAULT_DB_PATH, results_dir: str = "results"):
        """
    ...
    def _migrate_file_result(self, result: Dict[str, Any], session_id: str, db_session: Session):
        """
        ...
        if company_name:
            self.graph_store.add_file_to_company(file_id, company_name, session=db_session)
        ...
        if location_name or coords:
            self._add_location_to_file(file_id, location_name, coords, db_session)
    ...
    def _add_person_to_file(self, file_id: str, person_name: str, db_session: Session):
        """Add a person to a file."""
        if not person_name:
            return

        normalized = Person.normalize_name(person_name)
        person = db_session.query(Person).filter(Person.normalized_name == normalized).first()

        if not person:
            person = Person(name=person_name, normalized_name=normalized)
    ...
    def _add_location_to_file(
        ...
        location = self.graph_store.get_or_create_location(
            name=name, latitude=lat, longitude=lon, session=db_session
        )
    ...
    def _migrate_cost_report(self, file_path: Path, verbose: bool = True):
        ...
        if cost_summary:
            self.kv_store.hset(
                f"cost_report:{report_id}", "summary", cost_summary, namespace="stats"
            )

        # Store ROI summary
        roi_summary = data.get("roi_summary", {})
        if roi_summary:
            self.kv_store.hset(f"cost_report:{report_id}", "roi", roi_summary, namespace="stats")
        ...
        for feature_name, feature_data in feature_breakdown.items():
            self.kv_store.hset(
                f"feature_stats:{feature_name}", report_id, feature_data, namespace="stats"
            )
        ...
        if projections:
            self.kv_store.hset(
                f"cost_report:{report_id}", "projections", projections, namespace="stats"
            )
        ...
        if recommendations:
            self.kv_store.hset(
                f"cost_report:{report_id}", "recommendations", recommendations, namespace="stats"
            )
    ...
    def _store_generic_json(self, file_path: Path, verbose: bool = True):
        ...
        self.kv_store.set(key, data, namespace="metadata")
...
def run_migration(
    ...
    try:
        ...
        # Person-name validation columns (PERSON_NAME_VALIDATION_PLAN Phase 2).
        # Additive; legacy rows keep validation_scores='{}' (the "never
        # validated" marker) and review_status backfilled to 'auto_accepted'.
        if table_exists(conn, "people"):
            person_validation_columns = [
                ("review_status", "VARCHAR(20) DEFAULT 'auto_accepted'"),
                ("detection_confidence", "FLOAT"),
                ("validation_scores", "JSON DEFAULT '{}'"),
                ("validated_at", "DATETIME"),
            ]
            for col_name, col_def in person_validation_columns:
                if column_exists(conn, "people", col_name):
                    continue
                if dry_run:
                    print(f"  [DRY RUN] Would add {col_name} to people")
                else:
                    conn.execute(f"ALTER TABLE people ADD COLUMN {col_name} {col_def}")
                    print(f"  Added {col_name} to people")
                stats["columns_added"] += 1
            if not dry_run:
                conn.execute(
                    "UPDATE people SET review_status='auto_accepted' " "WHERE review_status IS NULL"
                )
        ...
        if table_exists(conn, "categories"):
            cursor = conn.execute(
                "SELECT id, full_path, name FROM categories WHERE canonical_id IS NULL"
            )
            rows = cursor.fetchall()
            if rows:
